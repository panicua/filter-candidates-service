...
class TelegramBot:
    def __init__(self):
        ...
        self.answers = TelegramAnswersDTO()
    ...
    async def ask_location(
        ...
        keyboard = [
            [
                ...
                for loc in list(LOCATION.keys())[i : i + 2]
            ]
            for i in range(0, len(LOCATION.keys()), 2)
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(
            "Please choose a location:", reply_markup=reply_markup
        )
    ...
    async def ask_experience(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> int:
        keyboard = [
            [
                InlineKeyboardButton(exp, callback_data=exp)
                for exp in list(EXPERIENCE.keys())[i : i + 2]
            ]
    ...
    async def ask_salary(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> int:
        keyboard = [
            [
                InlineKeyboardButton(salary, callback_data=salary)
                for salary in list(SALARY.keys())[i : i + 2]
            ]
    ...
    async def confirm_details(
        ...
        if update.message.text.lower() in ["yes", "y"]:
            ...
            #
            # Here spider will be called and results returned.
            #
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                ...
                self.ASKING_POSITION: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.ask_position
                    )
                ],
                ...
                self.CONFIRMING: [
                    MessageHandler(
                        filters.TEXT & ~filters.COMMAND, self.confirm_details
                    )
                ],
            },
